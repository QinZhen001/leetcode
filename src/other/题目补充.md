...
function calc(obj1, obj2) {
  ...
  if (obj1.year === obj2.year) {
    ...
    while (curYear <= obj2.year) {
      for (let i = curMonth; curYear === obj2.year ? i < obj2.month : i <= 12; i++) {
        arr.push(`${curYear}-${i}`);
      }
      curYear++;
      curMonth = 1;
...
给定两个排好序的数组 A,B,大小分别为 n,m。给出一个高效算法查找 A 中的哪些元素存在 B 数组中
...
const main = async () => {
  const [r1, r2, r3] = await Promise.all([batchedFn([1, 2, 3]), batchedFn([4, 5]), batchedFn([7, 8, 9])]);
...
[孤单的 QQ 号码怎么找](https://mp.weixin.qq.com/s?__biz=MjM5NTY1MjY0MQ==&mid=2650831669&idx=3&sn=1814f55abeb175cfb21a36632c895980&chksm=bd01ee7b8a76676dc084b87f7f0c99002b3006602a34d3b96f26a3eb42533aa88196f69cd9c5&mpshare=1&scene=1&srcid=0102MEFwyCSIfhFWhLpKQEec&sharer_sharetime=1641116415369&sharer_shareid=8d445fd79cda9a90618c95fff0c1211a#rd)

有 n 个 QQ 号码，除 1 个孤单的 QQ 号码外，其余的 QQ 号码都是成双成对的，求这个孤单的 QQ 号码，要求：时间复杂度为 O(n), 空间复杂度为 O(1).
...
function test(str) {
  ...
  let starts = []; // 开始标签文本 不带<>
  let ends = []; // 结束标签文本 不带</>
  let match = null;
  while ((match = startReg.exec(str)) != null) {
    starts.push(match[1]);
  }
  while ((match = endReg.exec(str)) != null) {
    ends.push(match[1]);
  }

  for (let i = 0; i < starts.length; i++) {
    if (starts[i] != ends[ends.length - i - 1]) {
      debugger;
      return false;
    }
  }
...
Object.defineProperty 会存在问题 无法监听到 delete data.count 属性删除
...
实现一个可以缓存其他函数的高阶函数 memoize。能够实现:当入参相同时，可以不经过计算，直接返回结果。
...
当前页面中元素节点的数量总和、元素节点的最大嵌套深度以及最大子元素个数，请用 JS 配合 原生 DOM API
实现该需求（不用考虑陈旧浏览器以及在现代浏览器中的兼容性，可以使用任意 浏览器的最新特性；不用考虑 shadow DOM）。比如在如下页面中运行后： */
...
// 写一个函数 // 输出： { totalElementsCount: 7, maxDOMTreeDepth: 4, maxChildrenCount: 3 }
```
