...
当我们不给数组赋初始值的时候，默认数组中的每一个元素都是 undefined，所以这时候我们拿数组元素去加减乘除计算，都会得到 NaN
...
回溯算法建立在 DFS 基础之上的，但不同的是在搜索的过程中，达到结束条件后，恢复状态，回溯上一层，再次搜索，因此我们可以这样子理解，回溯算法与 DFS 的区别就是有无状态重置。
...
- left 左指针
- right 右指针
- **注意右指针是在 left 循环中使用 while 循环 （理解）**
- 每次 left 循环中更新 ans 的值
...
有的时候我们需要在循环中频繁地计算数组 i 位置 -> j 的位置的和，这样纸非常的耗时，所以我们可以想办法缓存 sum，不需要每一次都计算。

举个例子：

> [和为 K 的子数组](https://leetcode-cn.com/problems/subarray-sum-equals-k/)

使用一个 totals 数组缓存数组的部分和
...
**我们可以看出来这个 2 是依赖它的左边，它的下边和它的左下角，所以要从后往前遍历**

### 返回值不一定是最后一项

通常情况下,动态规划的题目我们都是返回 dp 数组中的最后一项
...
需要 return 一个 TreeNode 类型的值，可以 return null，单写 return 相当于 return undefined
...
在对链表进行操作时，一种常用的技巧是添加一个哑节点（dummy node），它的 Ïnext 指针指向链表的头节点。这样一来，我们就不需要对头节点进行特殊的判断了。
...
当我们循环数组的长度大于 10000 时，算法的时间复杂度为 O(n^2) ，会超出题目的时间范围
